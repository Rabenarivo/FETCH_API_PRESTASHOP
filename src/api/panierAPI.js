...
// ── Helpers internes XML panier anonyme ──────────────────────

/**
 * Construit le XML de mise à jour d'un panier anonyme (id_customer=0).
 * Contrairement à construireXmlMiseAJourPanier, les adresses restent à 0.
 */
const construireXmlPanierAnonyme = ({ cart, idCart, rowsXml }) => {
  const idShop = enEntierPositif(getValue(cart?.id_shop, 1), 1);
  const idShopGroup = enEntierPositif(getValue(cart?.id_shop_group, 1), 1);
  const idLang = enEntierPositif(getValue(cart?.id_lang, 1), 1);
  const idCurrency = enEntierPositif(getValue(cart?.id_currency, 1), 1);
  const idGuest = enEntier(getValue(cart?.id_guest, 0), 0);
  const secureKey = String(getValue(cart?.secure_key, ''));
  const deliveryOption = String(getValue(cart?.delivery_option, ''));
...
// ── Panier anonyme (PrestaShop DB) ───────────────────────────

/**
 * Crée un panier anonyme dans PrestaShop (id_customer=0).
 * Tente d'abord de créer un enregistrement guest pour y rattacher le panier.
 * @returns {Promise<number>} - ID du panier créé
 */
export async function creerPanierAnonyme() {
  // Essayer de créer un guest pour rattacher le panier
  let idGuest = 0;
  try {
    const guestXml = `<?xml version="1.0" encoding="UTF-8"?>
<prestashop>
  <guest>
    <id_operating_system>0</id_operating_system>
    <id_web_browser>0</id_web_browser>
    <javascript>0</javascript>
    <mobile_theme>0</mobile_theme>
  </guest>
</prestashop>`;
    const { donnees } = await requeteApi('guests', { methode: 'POST', xml: guestXml });
    const guest = lireRessourceSimple(donnees, 'guest');
    idGuest = enEntier(getValue(guest?.id, 0), 0);
  } catch {
    // certaines configurations PS refusent l'endpoint guests, on continue sans
  }
...
<prestashop>
  ...
  const { donnees } = await requeteApi('carts', { methode: 'POST', xml: cartXml });
  const cart = lireRessourceSimple(donnees, 'cart');
  const idCart = enEntier(getValue(cart?.id, 0), 0);
  if (!idCart) throw new Error('Impossible de créer le panier anonyme');
  return idCart;
}

/**
 * Ajoute ou incrémente un produit dans un panier anonyme (id_customer=0).
 * N'effectue aucune résolution d'adresse ni de règle de taxe.
 * @param {number} idCart
 * @param {number} idProduit
 * @param {number} idDeclinaison
 * @param {number} quantite
 * @returns {Promise<number>} - ID du panier
 */
export async function ajouterProduitCartAnonyme(idCart, idProduit, idDeclinaison = 0, quantite = 1) {
  if (!idCart || !idProduit) throw new Error('Panier ou produit manquant');
  ...
  if (!cart) throw new Error(`Panier anonyme ${idCart} introuvable`);

  const existants = await lireProduitsCart(idCart);
  ...
  if (existant) {
    cartRows = existants.map((p) =>
      p.id_product === idProduit && p.id_product_attribute === idDeclinaisonFinale
        ? { ...p, quantity: p.quantity + quantite }
        : p
    );
  } else {
    cartRows = [
      ...existants,
      { id_product: idProduit, id_product_attribute: idDeclinaisonFinale, id_customization: 0, quantity: quantite, id_address_delivery: 0 },
    ];
  }

  const rowsXml = construireRowsXml(cartRows, 0);
  const xml = construireXmlPanierAnonyme({ cart, idCart, rowsXml });
  await requeteApi(`carts/${idCart}`, { methode: 'PUT', xml });
  return idCart;
}

/**
 * Supprime un produit d'un panier anonyme.
 * Si le panier devient vide, il est supprimé.
 * @returns {Promise<number|null>} - ID panier ou null si supprimé
 */
export async function supprimerProduitCartAnonyme(idCart, idProduit, idDeclinaison = 0) {
  if (!idCart || !idProduit) throw new Error('Panier ou produit manquant');
  ...
  const rowsXml = construireRowsXml(remaining, 0);
  const xml = construireXmlPanierAnonyme({ cart, idCart, rowsXml });
  await requeteApi(`carts/${idCart}`, { methode: 'PUT', xml });
  return idCart;
}
...
/**
 * Transfère le contenu d'un panier anonyme vers le panier d'un client réel,
 * puis supprime le panier anonyme.
 * @param {number} idCartAnonyme
 * @param {number} idClient
 * @returns {Promise<number>} - ID du panier client résultant (0 si panier vide)
 */
export async function transfererCartAnonyme(idCartAnonyme, idClient) {
  if (!idCartAnonyme || !idClient) return 0;

  let produits = [];
  try {
    produits = await lireProduitsCart(idCartAnonyme);
  } catch {
    return 0;
  }
  ...
  for (const item of produits) {
    try {
      idCartClient = await ajouterProduitCart(
        idCartClient || 0,
        item.id_product,
      ...
      console.warn(`[transfererCartAnonyme] produit ${item.id_product} ignoré :`, e?.message);
    }
  }

  // Supprimer le panier anonyme
  try {
    await requeteApi(`carts/${idCartAnonyme}`, { methode: 'DELETE' });
  } catch {
...
export async function obtenirOuCreerPanierClient(idClient) {
  ...
  // Chercher un panier non commande du client, en partant du plus recent.
  const { donnees } = await requeteApi(
    `carts?filter[id_customer]=[${idClient}]&display=[id,id_customer,id_shop,date_add]&limit=50&sort=[id_DESC]`
  );
  const carts = lireCollectionRessource(donnees, 'cart');

  for (const cart of carts) {
    const cartId = enEntier(getValue(cart?.id, 0), 0);
    if (!cartId) continue;

    // Verifier que ce panier n'est pas deja converti en commande.
    // Un panier converti retourne 500 sur GET/POST et doit etre ignore.
    try {
      ...
      if (!ordresExistants.length) {
        return {
          id: cartId,
          id_customer: enEntier(getValue(cart?.id_customer, 0), 0),
          id_shop: enEntier(getValue(cart?.id_shop, 1), 1),
          date_add: String(getValue(cart?.date_add, '')),
        };
      }
    } catch {
      // En cas d'erreur sur la verification, on ignore ce panier et on continue.
    }
  }
