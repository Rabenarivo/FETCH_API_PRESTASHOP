...
export const getProducts = async (idClient = 0) => {
  ...
  try {
    ...
    const [productsResponse, taxRateByGroup, categories] = await Promise.all([
      fetch(`${API_URL}/products?display=[id,name,price,wholesale_price,id_tax_rules_group,id_category_default]&sort=[id_ASC]&limit=200`, getFetchOptions()),
      buildTaxRateByGroupMap(taxContext).catch(() => new Map()),
      fetchResourceList('categories?display=[id,name]&limit=1000', 'category', 'categories').catch(() => []),
    ]);
    ...
    categories.forEach((category) => {
      const idCategory = Number(getValue(category?.id, 0)) || 0;
      if (!idCategory) return;
      categoryNameById.set(
        idCategory,
        getLangValue(category?.name, 1) || `Categorie #${idCategory}`
      );
    });
    ...
    return products.map((product) => {
      ...
      return {
        ...
        id_category_default: Number(getValue(product?.id_category_default, 0)) || 0,
        category_name:
          categoryNameById.get(Number(getValue(product?.id_category_default, 0)) || 0)
          || 'Sans categorie',
