...
export async function listerCommandes() {
  ...
  const commandes = lireCollectionRessource(resOrders.donnees, 'order').map((o) => ({
    ...
    totalPaye: getNumber(o.total_paid_real, 0),
    idCart: String(getValue(o.id_cart, '')),
...
/**
 * Compte les paniers qui n'ont pas encore ete convertis en commande.
 * @returns {Promise<number>}
 */
export async function lireNombreCartsSansCommande() {
  const [resCarts, resOrders] = await Promise.all([
    requeteApi('carts?display=[id]&limit=500'),
    requeteApi('orders?display=[id,id_cart]&limit=500'),
  ]);

  const tousLesCarts = lireCollectionRessource(resCarts.donnees, 'cart');
  const cartsCommandees = new Set(
    lireCollectionRessource(resOrders.donnees, 'order')
      .map((o) => String(getValue(o.id_cart, '')))
      .filter(Boolean)
  );

  return tousLesCarts.filter((c) => !cartsCommandees.has(String(getValue(c.id, '')))).length;
}
...
export async function changerEtatCommande(idCommande, idEtat) {
  ...
  const { donnees: donneesEtatInitial } = await requeteApi(`orders/${idCommande}?display=[id,current_state]`);
  const commandeActuelle = getCollection(donneesEtatInitial, 'order')[0] || null;
