...
const trouverProduitParReference = async (reference) => {
  ...
  const toProduitInfo = (produit) => ({
    ...
    prixHt: enNombre(getNumber(produit?.price, 0), 0),
...
const obtenirTauxTaxeDepuisGroupe = async (idGroupeTaxe) => {
  ...
  // NB: l'enveloppe XML est <tax_rules><tax_rule>...</tax_rule></tax_rules>,
  // donc on doit utiliser lireCollectionRessource (qui gère le pluriel)
  // au lieu de getCollection (qui ne lit que prestashop[nom]).
  ...
  return enNombre(getNumber(taxe?.rate, 0), 0);
};
...
const mettreAJourPrixDeclinaisonApi = async (idCombinaison, deltaPrix) => {
  if (!idCombinaison) return;

  const { texte } = await requeteApi(`combinations/${idCombinaison}?display=full`);
  let xml = String(texte || '');

  if (/<price(?:\s[^>]*)?>[^<]*<\/price>/.test(xml)) {
    xml = xml.replace(/<price(?:\s[^>]*)?>[^<]*<\/price>/, `<price>${Number(deltaPrix).toFixed(6)}</price>`);
  } else {
    xml = xml.replace('</combination>', `    <price>${Number(deltaPrix).toFixed(6)}</price>\n  </combination>`);
  }

  xml = xml.replace(/ xlink:href="[^"]*"/g, '');
  await requeteApi(`combinations/${idCombinaison}`, { methode: 'PUT', xml });
};
  ...
  for (const cellules of lignes) {
    ...
    try {
      ...
      if (prix_vente_ttc) {
        ...
        if (!(tauxTaxe > 0)) {
          // Sécurité : sans taux de taxe fiable on n'écrit pas un delta faux.
          prixCombiInvalide = true;
          warnings.push(
            `Ligne ${done}: taux de taxe introuvable pour groupe ${idGroupeTaxe} (${reference}/${karazany}) - prix combinaison non mis a jour`
          );
        } else {
          const prixHtCombi = prixTtcCombi / (1 + tauxTaxe / 100);
      ...
      if (idCombi) {
        if (prix_vente_ttc !== '' && prix_vente_ttc !== undefined && !prixCombiInvalide) {
          await mettreAJourPrixDeclinaisonApi(idCombi, deltaPrix);
        }
        console.log('[fichier2] declinaison existante reutilisee', {
          ...
          delta_prix_maj: deltaPrix.toFixed(6),
        });
      } else {
