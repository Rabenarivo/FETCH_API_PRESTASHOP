...
import { listerCommandes, lireNombreCartsSansCommande } from '../../api/commandeAPI';
...
function Dashboard() {
  ...
  const [nbCartsSansCommande, setNbCartsSansCommande] = useState(0);
  ...
  useEffect(() => {
    const charger = async () => {
      ...
      setErreur('');
      try {
        const [liste, nbCarts] = await Promise.all([
          listerCommandes(),
          lireNombreCartsSansCommande(),
        ]);
        setCommandes(liste);
        setNbCartsSansCommande(nbCarts);
  ...
  const commandesPayees = useMemo(
    () => commandesFiltrees.filter((commande) => Number(commande.totalPaye || 0) > 0),
    [commandesFiltrees]
  );

  const totalMontant = commandesPayees.reduce(
    (somme, commande) => somme + Number(commande.totalPaye || 0),
    0
  );

  const nbCommandes = commandesFiltrees.length;
  const nbCommandesPayees = commandesPayees.length;
    ...
    <div className="dashboard-container">
      ...
      <div className="dashboard-metrics">
        <article className="metric-card metric-primary">
          <span className="metric-label">Total des commandes payees</span>
          <strong className="metric-value">{chargement ? '…' : nbCommandesPayees}</strong>
        </article>
        <article className="metric-card metric-secondary">
          <span className="metric-label">TTC encaissé (commandes payées)</span>
          <strong className="metric-value">{chargement ? '…' : `${formatterMontant.format(totalMontant)} Ar`}</strong>
        </article>
        <article className="metric-card metric-tertiary">
          <span className="metric-label">Paniers en attente</span>
          <strong className="metric-value">{chargement ? '…' : nbCartsSansCommande}</strong>
        </article>
      </div>
      ...
      {!erreur && !chargement && (
        <div className="dashboard-detail">
          <div>
            <h3>{dateAffichage}</h3>
            <p>
              {nbCommandes} commande{nbCommandes !== 1 ? 's' : ''} au total, dont {nbCommandesPayees} payee{nbCommandesPayees !== 1 ? 's' : ''}.
            </p>
          </div>
